...
with open('{0}/R{1}-T60/rec_data/node0/data/ProcessMonitor'.format(path, input_rate),newline='') as processfile: 
    rows = csv.reader(processfile)
    for row in rows:
        process_data.append(row)
with open('{0}/R{1}-T60/rec_data/node0/data/CpuMonitor'.format(path, input_rate),newline='') as cpufile:
    rows = csv.reader(cpufile)
    for row in rows:
        cpu_data.append(row)
with open('{0}/R{1}-T60/rec_data/node0/data/IoMonitor'.format(path, input_rate),newline='') as iofile:
    rows = csv.reader(iofile)
    for row in rows:
        io_data.append(row)
with open('{0}/R{1}-T60/rec_data/node0/data/WebMonitor'.format(path, input_rate),newline='') as webfile:
...
if (start==0):
    plt.savefig('{0}/graph_all_time/R{1}-T60.png'.format(path, input_rate))
    plt.savefig('graph_all_time/R{0}-T60.png'.format(input_rate))

else:
    plt.savefig('{0}/graph/R{1}-T60.png'.format(path, input_rate))
