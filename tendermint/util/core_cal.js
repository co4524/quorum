...
const [CORE,INPUT_RATE, DURATION_TIME] = process.argv.slice(2, 5).map(it => parseInt(it))
const REPEAT = 1
const TEST_TIME = 1
const node_num = 4
...
for (let i = 1; i <= REPEAT; i++) {
  ...
  let post_process = require(`/home/caideyi/report/${CORE}core/R${INPUT_RATE}-T${DURATION_TIME}/rec_data/report${TEST_TIME}.json`)
  ...
  let log = `High-Core${CORE}\nTxRate,${INPUT_RATE}\nTPS,${stats.mean(tps)}\nLatency,${stats.mean(latency)}\nTPS_var,${stats.variance(tps)}\nLatency_var,${stats.variance(latency)}\n`
  console.log(log)
  if(INPUT_RATE===100){
    fs.existsSync(`${config.tendermint.path.graph}C${CORE}-H.txt`) && fs.unlinkSync(`${config.tendermint.path.graph}C${CORE}-H.txt`)
    fs.writeFileSync(`${config.tendermint.path.graph}C${CORE}-H.txt`,log)
  }
  else{
    let log2 = `High-Core${CORE}\n`
    let arr = fs.readFileSync(`${config.tendermint.path.graph}C${CORE}-H.txt`,"utf-8").split('\n').slice(0,-1)
    let arr2 = log.split('\n').slice(0,-1)
    for(let i = 1 ; i < arr.length ; i++){
      log2+=`${arr[i]},${arr2[i].split(',')[1]}\n`
    }
    fs.writeFileSync(`${config.tendermint.path.graph}C${CORE}-H.txt`,log2)
  }
}
