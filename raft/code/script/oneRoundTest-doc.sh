...
do
    ...
    if [ "$consensus" = "raft" ]; then
        ...
        --command="sh startRaft.sh data $web_socket $rpc $node_port $check_point 2" &   # [data_path, web_socket port, rpc port, node_port, check_point_port]
    else
        gcloud compute --project "$gcloud_proj_name" ssh --zone "$region" "$instance_name$node_index" \
        --command="sh startQuorum.sh data $web_socket $rpc $node_port $block_time 2" &   # [data_path, web_socket port, rpc port, node_port, block_time]
    fi
done
...
echo "register $tx_rate account"
node ../doc-tracking/preprocess $tx_rate
echo "sleep 10"
sleep 10
...
do
    slice=0
    node ../doc-tracking/workload $thread_tx_rate $duration_time $repeat $sleep_time $slice &
    slice=`expr $slice + $one_period`
done

# 2.active monitor - check all transaction done
node ../doc-tracking/txMonitor.js $tx_rate $duration_time $repeat
