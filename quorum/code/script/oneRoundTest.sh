...
nonce_increase=$(cat $path_configure | jq -r '.setting.nonce_increase_model')
queue_size=$(cat $path_configure | jq -r '.setting.global_queue_size')
# port
...
do
    ...
    if [ "$consensus" = "raft" ]; then
        ...
        --command="sh startRaft.sh data $web_socket $rpc $node_port $check_point $block_time $account_s $global_s $gas_target $gas_limit" &   # [data_path, web_socket port, rpc port, node_port, check_point_port, queue_size]
    else
		echo "Arg data $web_socket $rpc $node_port $block_time"
        gcloud compute --project "$gcloud_proj_name" ssh --zone "$region" "$instance_name$node_index" \
        --command="sh startQuorum.sh data $web_socket $rpc $node_port $block_time $account_s $global_s $gas_target $gas_limit" &   # [data_path, web_socket port, rpc port, node_port, block_time]
    fi
done
...
if [ "$nonce_increase" = "0" ]; then
    ...
    node ../workload_nonce_increase.js $tx_rate $duration_time $repeat $sleep_time $slice &
fi

# 2.active monitor - check all transaction done
node ../txMonitor.js $tx_rate $duration_time $repeat $path_configure
...
# 2.post_process
# if [ $consensus = "raft" ]; then
# node ../postprocess-raft.js
# else
node ../postprocess2.js
# fi
