...
for (let i = 1; i <= REPEAT; i++) {
  ...
  let post_process = require(`/home/caideyi/report/${MODEL}/R${INPUT_RATE}-T${DURATION_TIME}/report${TEST_TIME}.json`)
  ...
  console.log(log)
  if(INPUT_RATE===100){
    fs.existsSync(`${config.stellar.path.result}/${MODEL}/statistic.txt`) && fs.unlinkSync(`${config.stellar.path.result}/${MODEL}/statistic.txt`)
    fs.writeFileSync(`${config.stellar.path.result}/${MODEL}/statistic.txt`,log)
  }
  else{
    let log2 = `${MODEL}\n`
    let arr = fs.readFileSync(`${config.stellar.path.result}/${MODEL}/statistic.txt`,"utf-8").split('\n').slice(0,-1)
    let arr2 = log.split('\n').slice(0,-1)
    for(let i = 1 ; i < arr.length ; i++){
      log2+=`${arr[i]},${arr2[i].split(',')[1]}\n`
    }
    fs.writeFileSync(`${config.stellar.path.result}/${MODEL}/statistic.txt`,log2)
  }
}
