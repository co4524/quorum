...
async function monitor(){
    return new Promise(async function(resolve,reject){
        ...
        let interval = setInterval(async() => {
            ...
            console.log(`monitorTx BlkHeight ${current_block_height}, Txs_num ${block_transactions}, Now_value ${now_operation_number}, Expect_value ${start_operation_number+app.max_txs_in_round+1}`)
            if(block_transactions!=0 && first_block && consensus == 'quorum'){
                app.start_block_num = current_block_height
                first_block = false
            }
            if(now_operation_number===start_operation_number+app.max_txs_in_round+1){
